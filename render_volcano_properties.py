...
def coerce_our_structure_to_plotly_volcano(temp_fold_panda,temp_signifigance_panda,temp_metabolite):
    ...
    if only_plot_upper_triangle==True:
        ...
        temp_fold_panda[:]=np.where(np.arange(m)[:,None] >= np.arange(n),np.nan,temp_fold_panda)
        temp_signifigance_panda[:]=np.where(np.arange(m)[:,None] >= np.arange(n),np.nan,temp_signifigance_panda)
...
if __name__=="__main__":
    ...
    yes_signifigance_yes_fold=len(total_panda.loc[
        (total_panda['signifigance']<=signifigance_cutoff) & (total_panda['fold'].abs()>=fold_cutoff)
    ])
    yes_signifigance_no_fold=len(total_panda.loc[
        (total_panda['signifigance']<=signifigance_cutoff) & (total_panda['fold'].abs() < fold_cutoff)
    ])
    no_signifigance_no_fold=len(total_panda.loc[
        (total_panda['signifigance'] > signifigance_cutoff) & (total_panda['fold'].abs() < fold_cutoff)
    ])
    no_signifigance_yes_fold=len(total_panda.loc[
        (total_panda['signifigance'] > signifigance_cutoff) & (total_panda['fold'].abs() >= fold_cutoff)
    ])
